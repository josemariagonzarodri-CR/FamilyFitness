...
export default function App() {
  ...
  const dateProgRef = useRef(null)
  const dateTransRef = useRef(null)
  ...
  const cargarPrograma = async (userEmail) => {
    ...
    const { data: historialDataCompleto } = await supabase.from('sesiones_familiares')
      .select('id, fecha_registro, es_asistencia, dia_rutina, programa_id, ejercicios_rutina(nombre_ejercicio, series_ejercicio(numero_serie, peso_kg, repeticiones))')
  ...
  const crearPrograma = async () => {
    ...
    const { error } = await supabase.from('programas_entrenamiento').insert([{
      email_usuario: session.user.email, nombre_programa: formNombre, fecha_inicio: inicio.toISOString().split('T')[0], semanas_duracion: formSemanas, dias_por_semana: formDias, fecha_fin_teorica: fin.toISOString().split('T')[0], fecha_fin_estimada: fin.toISOString().split('T')[0], estado: 'ACTIVO'
    }]);
  ...
  const refrescarCatalogo = async () => {
    ...
    if (cat) setCatalogo(cat); else setCatalogo([]);
  }

  const agregarEjercicioCatalogo = async (e) => {
    e.preventDefault(); triggerHaptic();
    if(!catEj) return alert("Escribe un ejercicio");
    const { error } = await supabase.from('catalogo_rutina').insert([{ programa_id: programaActivo.id, dia_asignado: catDia, nombre_ejercicio: catEj, series_objetivo: catSeries, reps_objetivo: catReps.toString(), descanso_segundos: catDescanso }]);
    if(!error) { setCatEj(''); refrescarCatalogo(); }
  }

  const eliminarEjercicioCatalogo = async (idEjercicio) => { triggerHaptic(); await supabase.from('catalogo_rutina').delete().eq('id', idEjercicio); refrescarCatalogo(); }
  ...
  const exportarDatosCSV = () => {
    ...
    let csvContent = "Fecha_Registro,Dia_Entrenamiento,Nombre_Ejercicio,Numero_Serie,Peso_Normalizado_KG,Peso_Visual_LBS,Repeticiones,Volumen_KG,Estimacion_1RM_KG\n";
    historialGlobal.forEach(sesion => {
      ...
      if (sesion.ejercicios_rutina) {
        sesion.ejercicios_rutina.forEach(ej => {
          if (ej.series_ejercicio) {
            ej.series_ejercicio.forEach(serie => {
               const pesoKg = serie.peso_kg;
               const pesoLbs = (pesoKg * 2.20462).toFixed(1);
               const reps = serie.repeticiones;
               const volumen = (pesoKg * reps).toFixed(1);
               const oneRM = calcular1RM(pesoKg, reps).toFixed(1);
               const nombreLimpio = ej.nombre_ejercicio.replace(/,/g, ''); 
               csvContent += `${fecha},${dia},${nombreLimpio},${serie.numero_serie},${pesoKg},${pesoLbs},${reps},${volumen},${oneRM}\n`;
            });
          }
        });
      }
    ...
    link.setAttribute("download", `Data_TitaniumCore_Completa_${fDate(hoy)}.csv`);
  ...
  const iniciarEntrenamiento = () => {
    ...
    ejHoy.forEach(ej => {
      ...
      dataInicial[ej.id] = Array.from({ length: ej.series_objetivo }, () => ({ peso: pesoSugerido, reps: ej.reps_objetivo, completado: false }));
    });
  ...
  const updateSet = (ejId, setIndex, field, value) => {
    setWorkoutData(prev => {
      ...
      newData[ejId][setIndex] = { ...newData[ejId][setIndex], [field]: value };
      return newData;
    });
  }
  ...
  const finalizarEntrenamientoHoy = async () => {
    ...
    for (const ej of ejerciciosHoy) {
      ...
      if (completedSets.length > 0) {
          const { data: newEj } = await supabase.from('ejercicios_rutina').insert([{ sesion_id: newSession.id, nombre_ejercicio: ej.nombre_ejercicio }]).select().single();
          if (newEj) {
              const seriesToInsert = completedSets.map((s, index) => {
                  const pesoDigitado = parseFloat(s.peso) || 0;
                  const pesoParaSQL = unidad === 'lbs' ? (pesoDigitado / 2.20462) : pesoDigitado;
                  return { ejercicio_id: newEj.id, numero_serie: index + 1, peso_kg: pesoParaSQL, repeticiones: parseInt(s.reps) || 0 };
              });
              await supabase.from('series_ejercicio').insert(seriesToInsert);
          }
      }
    }
    alert("🏆 ¡Sesión, Pesos y Repeticiones archivados con precisión!");
            ...
            <div className="space-y-8 max-w-4xl mx-auto animate-fade-in stagger-1">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <form onSubmit={agregarEjercicioCatalogo} className="bg-white/[0.02] backdrop-blur-2xl border border-white/[0.05] p-8 rounded-[2rem] space-y-6 shadow-xl h-fit">
                    ...
                    <div>
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4 block">Ejercicio</label>
                      <input type="text" placeholder="Ej. Press de Banca..." value={catEj} onChange={e => setCatEj(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 font-bold outline-none focus:border-cyan-400/50 transition-colors text-white placeholder-slate-600" />
                    </div>
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div>
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 flex items-center">Reps</label>
                        <select value={catReps} onChange={e => {setCatReps(Number(e.target.value)); triggerHaptic();}} className="w-full bg-white/5 border border-white/10 rounded-2xl px-2 py-3.5 font-bold text-center text-white focus:border-cyan-400/50">
                          {[...Array(50)].map((_, i) => (<option key={i+1} value={i+1} className="bg-slate-900">{i + 1}</option>))}
                        </select>
                      </div>
                      <div>
                        ...
                        <select value={catDescanso} onChange={e => {setCatDescanso(Number(e.target.value)); triggerHaptic();}} className="w-full bg-white/5 border border-white/10 rounded-2xl px-2 py-3.5 font-bold text-center text-white focus:border-cyan-400/50">
                          <option value="30" className="bg-slate-900">30s</option><option value="45" className="bg-slate-900">45s</option><option value="60" className="bg-slate-900">60s</option><option value="90" className="bg-slate-900">90s</option><option value="120" className="bg-slate-900">120s</option><option value="180" className="bg-slate-900">180s</option>
                        </select>
                      </div>
                    </div>
                      ...
                      catalogo.map(ej => (
                        <div key={ej.id} className="bg-white/5 border border-white/5 p-4 rounded-2xl flex justify-between items-center hover:border-white/20 transition-colors">
                          <div>
                            ...
                            <div className="text-xs text-slate-400 font-bold ml-12">{ej.series_objetivo}x{ej.reps_objetivo} • ⏱️{ej.descanso_segundos}s</div>
                          </div>
          ...
          <div className="p-6 max-w-6xl mx-auto pt-8 md:pt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10">
                            ...
                            {isExpanded && sesion.es_asistencia && (
                              <div className="mt-4 pt-4 border-t border-white/10 space-y-3 animate-fade-in-fast cursor-default" onClick={e => e.stopPropagation()}>
                                {sesion.ejercicios_rutina?.map((ej, ejIdx) => {
                                  const totalEjKg = ej.series_ejercicio?.reduce((sum, s) => sum + (s.peso_kg * s.repeticiones), 0) || 0;
                                  ...
                                  return (
                                    <div key={ejIdx} className="bg-white/5 rounded-xl p-3 border border-white/5">
                                      <div className="flex justify-between items-center mb-3">
                                        <span className="text-xs font-black text-cyan-400 uppercase tracking-wider">{ej.nombre_ejercicio}</span>
                                        <span className="text-[10px] font-black text-white bg-black/40 px-2 py-1 rounded-lg border border-white/10">Vol: {totalEjDisplay} {unidad}</span>
                                      </div>
                                      <div className="space-y-1">
                                        {ej.series_ejercicio?.sort((a,b) => a.numero_serie - b.numero_serie).map((serie, sIdx) => {
                                          const pesoDisplay = unidad === 'lbs' ? (serie.peso_kg * 2.20462).toFixed(1).replace(/\.0$/, '') : serie.peso_kg;
                                          return (
                                            <div key={sIdx} className="flex justify-between text-[10px] text-slate-300 font-bold border-b border-white/5 pb-1.5 pt-1 last:border-0 last:pb-0">
                                              <span className="text-slate-500 tracking-widest uppercase">Set {serie.numero_serie}</span>
              ...
              <div className="md:col-span-5 flex flex-col gap-5 animate-fade-in stagger-2">
                    ...
                    <ul className="space-y-3 mb-6">
                      {resumenEjerciciosHoy.map(ej => (
                        <li key={ej.id} className="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-white/5">
                          ...
                          <span className="text-xs text-cyan-400 font-bold">{ej.series_objetivo}x{ej.reps_objetivo}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                ...
                <button onClick={registrarAusencia} className="w-full py-5 bg-transparent border border-white/10 text-slate-400 font-black uppercase tracking-[0.2em] rounded-[2rem] active:scale-95 transition-all hover:bg-white/5 hover:text-white flex items-center justify-center gap-2">⏸️ Registrar Ausencia </button>
            ...
            <div className="lg:col-span-8 space-y-6 flex flex-col">
              {ejerciciosHoy.length === 0 ? (
                <div className="flex-1 flex items-center justify-center p-10 bg-white/[0.02] border border-white/5 rounded-[2.5rem] animate-fade-in">
                  <p className="text-slate-500 text-center italic text-sm">El catálogo para el Día {diaToca} está vacío.<br/>Pausa el entrenamiento y agrega ejercicios en el editor.</p>
                </div>
              ) : (
                ejerciciosHoy.map((ej, idx) => {
                  const prKey = ej.nombre_ejercicio.trim().toUpperCase();
                  const maxPeso_kg = metricas[prKey]?.maxPeso || 0;
                  let recordPersonalDisplay = '';
                  if (maxPeso_kg > 0) recordPersonalDisplay = unidad === 'lbs' ? (maxPeso_kg * 2.20462).toFixed(1).replace(/\.0$/, '') : maxPeso_kg;
                    ...
                    <div key={ej.id} className="bg-white/[0.02] backdrop-blur-xl border border-white/[0.05] p-6 md:p-8 rounded-[2.5rem] hover:border-white/20 transition-all duration-300 animate-fade-in flex flex-col" style={{animationDelay: `${idx * 100}ms`}}>
                      <div className="mb-6">
                        <div className="flex flex-col items-start gap-2">
                          ...
                          {recordPersonalDisplay && (
                             <div className="bg-amber-500/10 border border-amber-500/30 text-amber-400 text-[10px] font-black uppercase px-3 py-1 rounded-lg flex items-center gap-2">🏆 PR Histórico: {recordPersonalDisplay} {unidad}</div>
                          )}
                        </div>
                      ...
                      <div className="space-y-3 flex-1">
                        <div className="flex text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] px-2 mb-2">
                          <div className="w-10 text-center">Set</div>
                          <div className="flex-1 text-center flex items-center justify-center">Peso ({unidad})</div>
                          <div className="flex-1 text-center flex items-center justify-center">Reps</div>
                          <div className="w-14 text-center">Status</div>
                        </div>
                        {workoutData[ej.id]?.map((set, i) => (
                          <div key={i} className={`flex items-center gap-3 p-2 rounded-2xl border transition-all duration-300 ${set.completado ? 'bg-cyan-500/10 border-cyan-500/30' : 'bg-black/40 border-white/5 hover:bg-white/5'}`}>
                            <div className={`w-10 text-center font-black ${set.completado ? 'text-cyan-400' : 'text-slate-500'}`}>{i + 1}</div>
                            <div className="flex-1 flex flex-col items-center">
                              ...
                              {mostrarConversion && set.peso && (<span className="text-[9px] text-cyan-500/50 mt-1 font-bold tracking-widest">{getValorConvertido(set.peso, unidad)}</span>)}
                            </div>
